...
# -*- coding: utf-8 -*-
"""
Evaluate the MedCorp RAG pipeline (Hybrid FAISS+BM25) with optional LLM-based
query reformulation integrated into the retrieval batch.

New flags:
  --use-reformulation         enable LLM-based query reformulation
  --reform-prompt-file        optional file path to a custom reformulation prompt
  --reform-max-tokens         max tokens for reformulation (default 120)
  --reform-temp               temperature for reformulation (default 0.0)
"""
...
def parse_letter(text: str, allowed_letters: List[str] = None) -> Optional[str]:
    ...
    # Prefer "Answer: X" near the end (handles CoT)
    lines = t.split('\n')
    last_lines = '\n'.join(lines[-5:])
...
def is_negation_question(q: str) -> bool:
    q_l = " " + (q or "")
    return any(tok in q_l for tok in NEGATION_TOKENS)

def make_hyde_prompt(question: str) -> str:
    return (
        "You are writing a brief medical reference note to help retrieve textbooks.\n"
        f"Question: {question}\n\n"
...
def make_hyde_prompt_for_option(question: str, opt_text: str) -> str:
    return (
        "You are writing a brief medical reference note to help retrieve textbooks.\n"
        f"Question: {question}\n"
        f"Candidate option: {opt_text}\n\n"
...
def make_hyde_prompts_for_example(question: str, options: Dict[str, str], hyde_mode: str) -> List[str]:
    """
    Build HyDE prompts for a single example.
    - hyde_mode: 'question' | 'option' | 'both'
    - For negation stems, return two 'question' prompts (likely-correct vs likely-incorrect)
    """
    prompts: List[str] = []
    neg = is_negation_question(question)
    if hyde_mode in ("question", "both"):
        if neg:
            prompts.append(
                make_hyde_prompt(
            ...
            prompts.append(make_hyde_prompt(question))
    if hyde_mode in ("option", "both"):
        for _, opt_text in sorted(options.items()):
            if opt_text.strip():
                prompts.append(make_hyde_prompt_for_option(question, opt_text.strip()))
    return prompts

# ----------------------------- Reformulation prompt + validator -----------------------------

REFORMULATION_PROMPT = """You are a medical information retrieval expert.
...
BAD_TOKENS = [
    "year-old", "years old", "patient", "presents", "comes",
    "man", "woman", "male", "female"
]

def validate_reformulated_query(q: str) -> bool:
    if not q or not q.strip():
        return False
    tokens = [t for t in re.findall(r"\S+", q.strip())]
    if not (5 <= len(tokens) <= 14):
        return False
    lower = q.lower()
    if any(bt in lower for bt in BAD_TOKENS):
        return False
...
def heuristic_fallback_query(question: str) -> str:
    tokens = [w.lower() for w in re.findall(r"[a-zA-Z]+", question) if len(w) > 4]
    # take most informative first 10 tokens
    return " ".join(tokens[:10]) if tokens else question[:120]

# ----------------------------- Batch Retrieval + Batched Rerank (ASYNC) -----------------------------

async def retrieve_batch_fast(
    ...
    min_lexical_overlap_fallback: float = 0.01,  # ✅ ADD THIS
    max_passages: int = 6,
    bm25_scope: str = "base",
    # batched reranker
    ...
    # HyDE
    use_hyde: bool = False,
    hyde_mode: str = "question",
    hyde_client: Optional[AsyncOllamaClient] = None,
    hyde_max_tokens: int = 120,
    hyde_temp: float = 0.2,
    # Reformulation
    use_reformulation: bool = False,
    reformulate_client: Optional[AsyncOllamaClient] = None,
    reform_prompt: str = REFORMULATION_PROMPT,
    reform_max_tokens: int = 120,
    reform_temp: float = 0.0,
) -> Tuple[List[List[Dict[str, Any]]], List[Optional[str]]]:
    """
    Returns per-example packed refs (list of dicts) in one batched pass, with optional HyDE and
    optional reformulation. Reformulated queries are appended as additional retrieval queries
    (kind == 'reform').

    Returns:
        Tuple of (out_refs, reformulated_queries)
        - out_refs: List of retrieved passages per example
        - reformulated_queries: List of reformulated query strings (one per example, None if not reformulated)
    """
    # --- Collect queries for all examples in the batch ---
    q_texts: List[str] = []
    q_meta: List[Dict[str, Any]] = []
    questions: List[str] = []   # per example question text
    options_per_example: List[Dict[str, str]] = []
    reformulated_queries: List[Optional[str]] = [None] * len(batch_examples)  # Track reformulated queries

    for ex_idx, ex in enumerate(batch_examples):
        question = (ex.get("question") or "").strip()
        questions.append(question)
        options  = make_options(ex)
        options_per_example.append(options)

        # base
        q_texts.append(question)
        q_meta.append({"ex_idx": ex_idx, "kind": "base"})

        # option-aware retrieval queries
        if option_aware:
        ...
        # expansions from dataset (if any)
        if use_expansions:
            for qe in sanitize_expansions(ex.get("query_expansions") or []):
                q_texts.append(f"{question} {qe}")
                q_meta.append({"ex_idx": ex_idx, "kind": "exp"})

    # --- HyDE: generate hypothetical notes (batched) and append as extra queries ---
    if use_hyde and hyde_client is not None:
        hyde_prompts: List[str] = []
        spans: List[Tuple[int, int, int]] = []  # (ex_idx, start, end) over hyde_prompts
        cursor = 0
        for ex_idx, (q, opts) in enumerate(zip(questions, options_per_example)):
            hp = make_hyde_prompts_for_example(q, opts, hyde_mode)
            if hp:
                hyde_prompts.extend(hp)
                spans.append((ex_idx, cursor, cursor + len(hp)))
                cursor += len(hp)

        if hyde_prompts:
            try:
                hyde_texts = await hyde_client.generate_batch(
                    hyde_prompts, max_tokens=hyde_max_tokens, temperature=hyde_temp, top_p=1.0
                )
            except Exception as e:
                print(f"[warn] HyDE generation failed, skipping HyDE this batch: {e}")
                hyde_texts = None

            if hyde_texts is not None:
                for ex_idx, s, e in spans:
                    for idx_h in range(s, e):
                        htxt = (hyde_texts[idx_h] or "").strip()
                        if not htxt:
                            continue
                        # Append each HyDE note as an independent retrieval query
                        q_texts.append(htxt)
                        q_meta.append({"ex_idx": ex_idx, "kind": "hyde"})

    # --- REFORMULATION: batch reformulate the base questions (if enabled) ---
    if use_reformulation and reformulate_client is not None:
        reform_prompts: List[str] = []
        reform_spans: List[Tuple[int, int, int]] = []  # (ex_idx, start, end)
        cursor = 0
        # build one reform prompt per example (only for base question)
        for ex_idx, q in enumerate(questions):
            rp = reform_prompt.format(question=q)
            reform_prompts.append(rp)
            reform_spans.append((ex_idx, cursor, cursor+1))
            cursor += 1

        if reform_prompts:
            try:
                reform_texts = await reformulate_client.generate_batch(
                    reform_prompts, max_tokens=reform_max_tokens, temperature=reform_temp, top_p=1.0
                )
            except Exception as e:
                print(f"[warn] Reformulation generate_batch failed: {e}")
                reform_texts = None

            if reform_texts is not None:
                for idx_r, rtxt in enumerate(reform_texts):
                    ex_idx, s, e = reform_spans[idx_r]
                    out = (rtxt or "").strip().replace("\n", " ")
                    if not out:
                        out = heuristic_fallback_query(questions[ex_idx])
                    if not validate_reformulated_query(out):
                        # fallback
                        out = heuristic_fallback_query(questions[ex_idx])
                    # Store the reformulated query for this example
                    reformulated_queries[ex_idx] = out
                    # Append reformulated query as independent retrieval query
                    q_texts.append(out)
                    q_meta.append({"ex_idx": ex_idx, "kind": "reform"})
    ...
    return out_refs, reformulated_queries
...
async def evaluate_async(
    ...
    # Reformulation
    use_reformulation: bool = False,
    reform_prompt_file: Optional[str] = None,
    reform_max_tokens: int = 120,
    reform_temp: float = 0.0,
    # Streaming scoring
    ...
    # LLM client for answering / used also by HyDE and reformulation
    llm = AsyncOllamaClient(llm_model, max_workers=workers, base_url=ollama_base_url)

    # LLM client for HyDE (can be same model)
    hyde_client = None
    if use_hyde:
        hyde_model_eff = hyde_model or llm_model
        hyde_client = AsyncOllamaClient(hyde_model_eff, max_workers=workers, base_url=ollama_base_url)

    # Reformulation client uses same LLM instance (llm) unless user wants separate
    reform_client = llm if use_reformulation else None

    # Load custom reformulation prompt if provided
    if reform_prompt_file:
        try:
            with open(reform_prompt_file, "r", encoding="utf-8") as rf:
                reform_prompt_text = rf.read()
                print(f"Loaded reformulation prompt from {reform_prompt_file} ({len(reform_prompt_text.split())} tokens approx)")
        except Exception as e:
            print(f"[warn] Failed to load reform prompt file: {e}; using default prompt.")
            reform_prompt_text = REFORMULATION_PROMPT
    else:
        reform_prompt_text = REFORMULATION_PROMPT
    ...
    # Update the save directory dynamically based on the input model name
    SAVE_DIR = os.path.join(
        PROJECT_ROOT,
        "evaluation_results",
        "medqa_usmle",
        "rag",
        embed_model_query.replace('/', '_').replace(':', '_')
    )
    ensure_dir(SAVE_DIR)
    ...
    with open(preds_path, "w", encoding="utf-8") as pred_file, \
        ...
        for start_i in tqdm(range(0, len(data), retrieval_batch), desc="Retrieving (batched)"):
            ...
            t0 = time.time()
            batch_refs, batch_reformulated_queries = await retrieve_batch_fast(
                ...
                reformulate_client=reform_client,
                reform_prompt=reform_prompt_text,
                reform_max_tokens=reform_max_tokens,
                reform_temp=reform_temp,
            )
            t1 = time.time()
            ...
            for ex_idx, (ex, refs) in enumerate(zip(batch_examples, batch_refs)):
                question = (ex.get("question") or "").strip()
                options  = make_options(ex)
                gold     = gold_letter(ex)
                prompt   = build_prompt(question, options, refs, use_cot=use_cot)
                local_prompts.append(prompt)
                reformulated_query = batch_reformulated_queries[ex_idx] if ex_idx < len(batch_reformulated_queries) else None
                local_metas.append({
                    "example": ex,
                    "gold": gold,
                ...
                for sb in range(0, len(local_prompts), batch_size):
                    ...
                    for j, (resp, meta) in enumerate(zip(responses, smetas)):
                        ...
                        retrieved_content = {
                            ...
                            "reformulated_query": meta.get("reformulated_query"),
                            "retrieved_passages": [
                                {
    ...
    metrics = {
        ...
        "settings": {
            ...
            "use_hyde": use_hyde,
            "hyde_mode": hyde_mode,
            "hyde_model": hyde_model or llm_model,
            "hyde_max_tokens": hyde_max_tokens,
            "hyde_temp": hyde_temp,
            # Reformulation
            "use_reformulation": use_reformulation,
            "reform_prompt_file": reform_prompt_file,
            "reform_max_tokens": reform_max_tokens,
            "reform_temp": reform_temp,
        },
        "system_info": {
...
def main():
    ...
    ap.add_argument("--eval-path", type=str, default=os.path.join(PROJECT_ROOT, "data", "medQA USMLE","questions","US" ,"test.jsonl"), help="Path to evaluation data (MedMCQA or MedQA format)")
    ap.add_argument("--save-dir", type=str, default=os.path.join(PROJECT_ROOT, "evaluation_results", "baseline_rag_reformulation_reranked"))
    ap.add_argument("--ollama-base-url", type=str, default=os.environ.get("OLLAMA_BASE_URL", "http://172.25.208.1:11434"))

    # Embeddings
    ap.add_argument("--embed-model-query", type=str, default="kronos483/MedEmbed-large-v0.1:latest",
    ...
    ap.add_argument("--bm25-scope", type=str, choices=["base", "all", "none"], default="base",
                    help="BM25 only for base question, for all queries (incl HyDE/option), or disabled.")
    ...
    ap.add_argument("--use-hyde", action="store_true",
                    help="Enable HyDE: generate hypothetical notes for retrieval")
    ap.add_argument("--hyde-mode", type=str, default="question", choices=["question", "option", "both"],
                    help="Generate HyDE for the question, for each option, or both")
    ap.add_argument("--hyde-max-tokens", type=int, default=120,
                    help="Max tokens for HyDE generations")
    ap.add_argument("--hyde-temp", type=float, default=0.2,
                    help="Temperature for HyDE generations")
    ap.add_argument("--hyde-model", type=str, default=None,
                    help="LLM to use for HyDE generations (defaults to --llm-model)")

    # Reformulation flags (new)
    ap.add_argument("--use-reformulation", action="store_true", help="Enable LLM-based query reformulation (adds 'reform' queries)")
    ap.add_argument("--reform-prompt-file", type=str, default=None, help="Optional custom reformulation prompt file")
    ap.add_argument("--reform-max-tokens", type=int, default=120, help="Max tokens for reformulation LLM")
    ap.add_argument("--reform-temp", type=float, default=0.0, help="Temperature for reformulation LLM")
    ...
    asyncio.run(
        evaluate_async(
            ...
            hyde_temp=args.hyde_temp,
            # Reformulation
            use_reformulation=args.use_reformulation,
            reform_prompt_file=args.reform_prompt_file,
            reform_max_tokens=args.reform_max_tokens,
            reform_temp=args.reform_temp,
